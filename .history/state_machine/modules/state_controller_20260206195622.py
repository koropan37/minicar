...
class StateController:
    ...
    def _handle_recover(self, L, FL, C, FR, R, pattern):
        ...
        if self.state_duration < 0.5:
            steer = SERVO_SLIGHT_RIGHT if L < WALL_CLOSE else SERVO_CENTER
            return State.RECOVER, steer, THROTTLE_REVERSE
        ...
        if self.state_duration > 0.4:
            if L < WALL_CLOSE:
                return State.RECOVER, SERVO_SLIGHT_RIGHT, THROTTLE_REVERSE
            if R < WALL_CLOSE:
