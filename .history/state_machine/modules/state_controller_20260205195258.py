...
class StateController:
    ...
    def _handle_wall_follow(self, L, FL, C, FR, R, pattern):
        ...
        # 右コーナー検出（右が大きく開けた & 正面に壁）
        if pattern['right_corner_detected']:
            return State.RIGHT_TURN, SERVO_RIGHT, THROTTLE_SLOW

        # 左コーナー検出（正面が近い & 右壁あり）
        if C < FRONT_BLOCKED_THRESHOLD and R < WALL_FAR:
            return State.LEFT_TURN, SERVO_LEFT, THROTTLE_SLOW

        # PID制御で右壁沿い走行
        error = R - TARGET_RIGHT_DISTANCE

        # 右前が近すぎる場合は、壁に寄りすぎているので左に修正
        if FR < TARGET_RIGHT_DISTANCE * 1.5:
            steering = SERVO_CENTER - 20  # 左へ
            error = -999
        else:
            steering = SERVO_CENTER + (error * 0.15)
